...
class ProductoServiceTest {
	...
	// Para eliminar un producto, debe buscarlo, traerlo, cambiar atributo "Estado"
	// de "Activo" a "Inactivo" y
	// luego guardarlo (Servicio), controlador invoca método de eliminación y
	// retorna status OK si la operación se realizó exitosamente.

	// Buscar y obtener el producto
	// Caso exitoso
	@Test
	void siDeseaEliminarUnProductoEntoncesProductoServiceBuscaElProductoMedianteSuIdYLoRetorna()
		...
		// Assert
		assertNotNull(productoObtenido);
		assertAll("productoObtenido", () -> assertEquals(3, productoObtenido.getCodigo()),
				() -> assertEquals("Tierra Biologica Compost".toLowerCase(),
						productoObtenido.getNombre().toLowerCase()),
				() -> assertEquals("ANASAC".toLowerCase(), productoObtenido.getMarca().toLowerCase()),
				() -> assertEquals(
						"Producto natural, hecho a partir de la compostación de residuos orgánicos".toLowerCase(),
	...
	// Caso fallido (no encuentra el producto y retorna null, manejado mediante
	...
	void siDeseaEliminarUnProductoYProductoServiceLoBuscaPeroNoLoEncuentraEntoncesLanzaProductNotFoundException() {
		...
		// Act + Assert
		assertThrows(ProductNotFoundException.class, () -> productService.searchProduct(3));
	}

	// Guardar cambio en atributo "Estado" de Activo a Inactivo
	// Caso exitoso
	@Test
	void siDeseaEliminarProductoYEsteEsEncontradoEntoncesCambiaAtributoEstadoYGuardaLosCambios()
			throws ProductNotFoundException {
		// Arrange
		Producto productoBuscado = new Producto(3, "Tierra Biologica Compost", "ANASAC",
				"Producto natural, hecho a partir de la compostación de residuos orgánicos", 4990, 20, "Activo");
